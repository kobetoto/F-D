...
std::vector<PairNode>   fj_sort_winners(const std::vector<PairNode> &pairs_v, size_t total_ids)
{
    ...
    std::vector<size_t> posById(total_ids, 0); //for jacob F.Legend
    ...
    for(size_t i = 0; i < order.size(); i++){
        ...
        //insertion index Jacob 
        size_t jacob_index = order[i];

        //if orphan (winners_size+2 == convention !=  winners_size+1 == normal elem)
        ...
        if(has_orphan && jacob_index == (winners_size + 2)){
            ...
            rebuild_posById(S, posById);// insertion = decallage = MAJ::posById == S 
        }
        else{
            //y3 -> W[1], y4 -> W[2] 
            size_t winner_index = jacob_index-2; //convertis l'index y_ en index w_
            if(winner_index >= winners_size)
                error("bad Y index");

            PairNode wnode = W[winner_index]; //recup le winner associe au perdant de ce tour
